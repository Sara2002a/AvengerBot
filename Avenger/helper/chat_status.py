...
from Avenger import (BOT_ID, GROUP_ANONYMOUS_BOT, SUDO_USERS, AvengerCli,
                    TELEGRAM_SERVICES_IDs)
from Avenger.database.connection_mongo import GetConnectedChat
...
async def isBotAdmin(message: Message, chat_id=None, silent=False) -> bool:
    ...
    GetData  = await AvengerCli.get_chat_member(
        chat_id=chat_id,
        user_id=BOT_ID
    )
...
async def isUserBanned(chat_id: int, user_id: int) -> bool:
    ...
    """
    data_list = await AvengerCli.get_chat_members(
        chat_id=chat_id,
        filter='kicked'
        )
