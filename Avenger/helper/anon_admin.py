...
from Avenger import GROUP_ANONYMOUS_BOT, AvengerCli
from Avenger.database.chats_settings_mongo import get_anon_setting
from Avenger.helper.chat_status import anon_admin_checker


def anonadmin_checker(func):
    """Generates a verification button for the anon admin in the if /anonadmin is off
    Args:
        func ([type]): pass client: AvengerCli, message: Message
    """
    async def wrap(client, message: Message):
        ...
        @AvengerCli.on_callback_query(filters.create(lambda _, __, query: 'anonAdminConfirm_' in query.data))
        async def anon_callback(client: AvengerCli, callback_query: CallbackQuery):
